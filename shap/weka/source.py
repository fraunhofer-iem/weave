...
classifier = Classifier(classname="weka.classifiers.meta.ClassificationViaRegression", options=[
    "-W", "weka.classifiers.trees.RandomForest", "-do-not-check-capabilities", "--", "-P", "100", "-I", "100",
    "-num-slots", "1", "-do-not-check-capabilities", "-K", "0", "-M", "8.0", "-V", "0.001", "-S", "1"
])
...
fig = plt.figure()
shap.plots.beeswarm(global_exp[:, :dataset.num_attributes - 1], show=False)
...
global_shap_df = pd.DataFrame(global_exp.values, columns=feature_names)
...
for i in range(0, len(test_dataset_np)):
    shap.plots.waterfall(local_exp[i,:dataset.num_attributes - 1], show=False)
    ...
    local_shap_df = pd.DataFrame([local_exp[i, :].values], columns=feature_names)
...
# Log all stats from the explainer
local_shap_df = pd.DataFrame(local_exp.values, columns=feature_names)
