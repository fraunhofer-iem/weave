...
fig = plt.figure()
shap.plots.beeswarm(global_exp[:, :dataset.num_attributes - 1, 0], show=False)
ax = plt.gca()
yticklabels = ax.get_yticklabels()
for label1 in yticklabels:
    if "Sum of 95 other features" in label1.get_text():
        label1.set_text("∑95RemFeat")
ax.set_yticklabels(yticklabels)
plt.xlabel('SHAP Value')
plt.savefig(f'results/{label}-global-label_none.pdf', bbox_inches='tight', dpi=300)
plt.close()
...
ax.set_yticklabels(yticklabels)
plt.xlabel('SHAP Value')
plt.savefig(f'results/{label}-global-label_target.pdf', bbox_inches='tight', dpi=300)
plt.close()
logger.info(f"Saved SHAP plot for {label}")

shap.plots.beeswarm(global_exp[:, :dataset.num_attributes - 1, 2], show=False)
ax = plt.gca()
yticklabels = ax.get_yticklabels()
for label1 in yticklabels:
    if "Sum of 95 other features" in label1.get_text():
...
ax.set_yticklabels(yticklabels)
plt.xlabel('SHAP Value')
plt.savefig(f'results/{label}-global-label_input.pdf', bbox_inches='tight', dpi=300)
plt.close()
logger.info(f"Saved SHAP plot for {label}")

# Log all stats from the explainer
logger.info("--------------- Global Statistics ---------------")
logger.info("--------------- Label 0 (NONE)---------------")
global_shap_df = pd.DataFrame(global_exp[:, :, 0].values, columns=feature_names)
...
logger.info("--------------- Label 1 (TARGET)---------------")
global_shap_df = pd.DataFrame(global_exp[:, :, 1].values, columns=feature_names)
logger.info("Shap Values: \n" + global_shap_df.abs().mean().sort_values(ascending=False).to_string())
logger.info("Shap Summary: \n" + str(global_shap_df.describe()))

logger.info("--------------- Label 2 (INPUT)---------------")
global_shap_df = pd.DataFrame(global_exp[:, :, 2].values, columns=feature_names)
logger.info("Shap Values: \n" + global_shap_df.abs().mean().sort_values(ascending=False).to_string())
logger.info("Shap Summary: \n" + str(global_shap_df.describe()))
...
local_explainer = shap.Explainer(weka_predict, dataset_np, feature_names=feature_names, seed=40)
...
for i in range(0, len(test_dataset_np)):
    shap.plots.waterfall(local_exp[i,:dataset.num_attributes - 1, 0], show=False)
    plt.savefig(f'{output_dir}/{label}-local-instance-{i}-0.pdf', bbox_inches='tight', dpi=300)
    plt.close()

    shap.plots.waterfall(local_exp[i, :dataset.num_attributes - 1, 1], show=False)
    plt.savefig(f'{output_dir}/{label}-local-instance-{i}-1.pdf', bbox_inches='tight', dpi=300)
    plt.close()

    shap.plots.waterfall(local_exp[i, :dataset.num_attributes - 1, 2], show=False)
    plt.savefig(f'{output_dir}/{label}-local-instance-{i}-2.pdf', bbox_inches='tight', dpi=300)
    plt.close()
    logger.info(f"Saved SHAP force plot for instance {i}")

    # Save local shap values to csv files
    instance_csv_path = f'{csv_path}/instance_{i}_label_none.csv'
    local_shap_df = pd.DataFrame([local_exp[i, :, 0].values], columns=feature_names)
    ...
    instance_csv_path = f'{csv_path}/instance_{i}_label_target.csv'
    local_shap_df = pd.DataFrame([local_exp[i, :, 1].values], columns=feature_names)
    local_shap_df = local_shap_df.T.reset_index()
    local_shap_df.columns = ['Feature', 'SHAP Value']
    local_shap_df.sort_values(by='SHAP Value', ascending=False).to_csv(instance_csv_path, index=False)

    instance_csv_path = f'{csv_path}/instance_{i}_label_input.csv'
    local_shap_df = pd.DataFrame([local_exp[i, :, 2].values], columns=feature_names)
    local_shap_df = local_shap_df.T.reset_index()
    local_shap_df.columns = ['Feature', 'SHAP Value']
    local_shap_df.sort_values(by='SHAP Value', ascending=False).to_csv(instance_csv_path, index=False)
...
logger.info("--------------- Label 0 (N0NE)---------------")
local_shap_df = pd.DataFrame(local_exp[:, :, 0].values, columns=feature_names)
...
logger.info("--------------- Label 2 (INPUT)---------------")
local_shap_df = pd.DataFrame(local_exp[:, :, 2].values, columns=feature_names)
logger.info("Shap Values: \n" + local_shap_df.abs().mean().sort_values(ascending=False).to_string())
logger.info("Shap Summary: \n" + str(local_shap_df.describe()))
