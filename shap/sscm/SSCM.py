...
plt.savefig(f'results/{label}-0-global.pdf', bbox_inches='tight', dpi=300)
plt.close()
logger.info(f"Saved SHAP plot for {label}")

fig = plt.figure()
shap.plots.beeswarm(exp[:, :dataset.num_attributes - 1, 1], show=False)
plt.xlabel('SHAP Value')
plt.savefig(f'results/{label}-1-global.pdf', bbox_inches='tight', dpi=300)
plt.close()
logger.info(f"Saved SHAP plot for {label}")

fig = plt.figure()
shap.plots.beeswarm(exp[:, :dataset.num_attributes - 1, 2], show=False)
plt.xlabel('SHAP Value')
plt.savefig(f'results/{label}-2-global.pdf', bbox_inches='tight', dpi=300)
plt.close()
logger.info(f"Saved SHAP plot for {label}")
